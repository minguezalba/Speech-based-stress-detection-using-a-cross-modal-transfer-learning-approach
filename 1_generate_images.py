...
def generate_images(method_image):
    ...
    for f, ((f_path, f_name), (l_path, l_name)) in enumerate(zip(audio_files, label_files)):
        print(f'Loading audio file {f+1}/{len(audio_files)}: {f_name}')
        ...
        for i, (audio_frame, label) in enumerate(zip(audio_manager.data, labels_manager.data)):
            print(f'\t - Frame {i+1} / {audio_manager.n_frames}')
